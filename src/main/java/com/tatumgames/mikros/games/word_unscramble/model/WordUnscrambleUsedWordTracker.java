...
public class WordUnscrambleUsedWordTracker {
  ...
  /**
   * Clears all used-word tracking for a guild.
   *
   * <p>Used when `/admin-scramble-setup` is re-run and the guild needs a full clean start.
   *
   * @param guildId the guild ID
   */
  public void clearGuild(String guildId) {
    ...
    usedWords.remove(guildId);
    logger.info("Cleared used-word tracking for guild {}", guildId);
  }
