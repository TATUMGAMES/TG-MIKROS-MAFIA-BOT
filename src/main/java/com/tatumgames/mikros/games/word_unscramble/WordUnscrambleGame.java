...
public class WordUnscrambleGame implements WordUnscrambleInterface {
    ...
    static {
        ...
        LEVEL_WORDS.put(3, Arrays.asList(
                ...
                "ENCHANT", "BATTLE", "COMBAT", "FIGHT", "STRIKE", "SLASH", "PIERCE", "CRUSH", "BURN", "FREEZE", "SHOCK",
                "HEALER", "TANKER", "DAMAGER", "SUPPORT", "BUFFER", "DEBUFFER", "CONTROL", "AOE", "SINGLE",
                "LEGEND", "EPIC", "RARE", "COMMON", "UNIQUE", "MYTHIC", "ARTIFACT", "RELIC", "TREASURE", "HOARD", "HORDE"
        ));
        ...
        LEVEL_WORDS.put(5, Arrays.asList(
                ...
                "CAMPAIGN", "DEFENSE", "OFFENSE", "POWER", "ABILITY"
        ));

        // Level 6: Medium-Long (8-10 letters)
        LEVEL_WORDS.put(6, Arrays.asList(
                "CHARACTER", "INVENTORY", "SOUNDTRACK", "ENCHANTING", "MINIMAP", "CHECKPOINT", "PLAYTHROUGH",
                "NARRATIVE", "STORYLINE", "PROTAGONIST", "ANTAGONIST", "QUESTLINE", "SIDEMISSION", "MAINQUEST", "SIDEQUEST",
                "EQUIPMENT", "WEAPONRY", "ACCESSORY", "CONSUMABLE", "MATERIAL", "RESOURCE", "CURRENCY"
        ));
        ...
        LEVEL_WORDS.put(7, Arrays.asList(
                ...
                "BLACKSMITH", "MERCHANT", "GUARDIAN", "SENTINEL", "CHAMPION", "LEGENDARY",
                "ADVENTURER", "EXPLORER", "HOARDER", "COLLECTOR", "GATHERER", "HUNTER", "TRACKER",
        ...
        LEVEL_WORDS.put(9, Arrays.asList(
                "TRANSMISSION", "COMPETITIVE", "INNOVATION", "ADVENTURING", "STREAMSNIPING",
                "CROSSPLAY", "MULTIPLATFORM", "EXCLUSIVE", "TIMEDEXCLUSIVE", "LAUNCHTITLE", "INDIE", "AAA",
        ...
        LEVEL_WORDS.put(10, Arrays.asList(
                ...
                "PERFORMANCE", "FRAMERATE", "RESOLUTION", "TEXTURE", "SHADER", "LIGHTING", "RENDERING"
        ));
     ...
     * @return a random word or phrase from that level
     */
    public static String getRandomWordForLevel(int level, String guildId, WordUnscrambleUsedWordTracker wordTracker) {
     ...
     * @return the new game session
     */
    public WordUnscrambleSession startNewSession(String guildId, int level, WordUnscrambleUsedWordTracker wordTracker) {
        String word = getRandomWordForLevel(level, guildId, wordTracker);
        WordUnscrambleSession session = new WordUnscrambleSession(guildId, WordUnscrambleType.WORD_UNSCRAMBLE, Instant.now(), word, level);
    ...
    public WordUnscrambleResult handleAttempt(WordUnscrambleSession session, String userId, String username, String input) {
        ...
        boolean alreadyWon = session.getResults().stream()
                .anyMatch(r -> r.userId().equals(userId) && r.isCorrect());
        ...
        if (isCorrect) {
            ...
            boolean isFirstSolver = session.getResults().stream()
                    .noneMatch(r -> r.isCorrect());
        ...
        WordUnscrambleResult result = new WordUnscrambleResult(userId, username, input, totalScore, bonus + firstSolverBonus, isCorrect, Instant.now());
    ...
    /**
     * Generates a hint for a word/phrase.
     * Randomly selects from: first letter, last letter, word length, vowels, or category.
     *
    ...
    /**
     * Generates a hint for a word/phrase (for levels 6+).
     * Randomly selects from: first letter, last letter, word length, vowels, or category.
     *
     * @param word  the word/phrase
     * @param level the level
    ...
    private String generateCategoryHint(String word, int level) {
        ...
        } else if (lowerWord.contains("weapon") || lowerWord.contains("sword") || lowerWord.contains("armor")) {
            return "Related to **equipment or weapons**";
        } else if (lowerWord.contains("quest") || lowerWord.contains("mission") || lowerWord.contains("story")) {
            return "Related to **quests or story**";
        } else if (lowerWord.contains("player") || lowerWord.contains("character") || lowerWord.contains("class")) {
            return "Related to **characters or classes**";
        } else if (lowerWord.contains("skill") || lowerWord.contains("ability") || lowerWord.contains("spell")) {
            return "Related to **skills or abilities**";
        } else if (lowerWord.contains("game") || lowerWord.contains("mode") || lowerWord.contains("play")) {
            return "Related to **gameplay or modes**";
        } else if (lowerWord.contains("level") || lowerWord.contains("dungeon") || lowerWord.contains("zone")) {
            return "Related to **locations or levels**";
        } else if (lowerWord.contains("item") || lowerWord.contains("loot") || lowerWord.contains("treasure")) {
    ...
    /**
     * Scrambles a single word with level-aware algorithm.
     * For levels 6+, uses enhanced scrambling with minimum displacement.
     *
     ...
     * Ensures minimum character displacement and uses multiple shuffle passes.
     *
     * @param word the word to scramble
     * @return the scrambled word
     */
