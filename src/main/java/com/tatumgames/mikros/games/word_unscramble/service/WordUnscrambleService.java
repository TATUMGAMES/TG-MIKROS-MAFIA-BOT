...
public class WordUnscrambleService {
  ...
  /**
   * Performs a full guild clean-start for Word Unscramble.
   *
   * <p>Clears active session, progression, per-player stats, and used-word history so the re-setup
   * behaves like a fresh guild.
   *
   * @param guildId the guild ID
   */
  public void resetGuildData(String guildId) {
    if (guildId == null) {
      return;
    }

    activeSessions.remove(guildId);
    wordUnscrambleProgression.remove(guildId);

    // Clear all per-player stats for the guild.
    String prefix = guildId + "_";
    playerStats.keySet().removeIf(key -> key.startsWith(prefix));

    usedWordTracker.clearGuild(guildId);

    logger.info("Reset Word Unscramble guild data for {}", guildId);
  }
