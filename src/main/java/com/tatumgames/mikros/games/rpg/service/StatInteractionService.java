...
/**
 * Service for handling stat-gated world interactions during exploration. These encounters (10-15%
 * chance) require stat checks and provide meaningful success/failure outcomes.
 */
public class StatInteractionService {
  ...
  /**
   * Rolls for a stat-gated interaction. Only triggers if character is Level 10+ and hasn't exceeded
   * the limit (3 per interaction type).
   *
  ...
  public StatInteractionType rollForStatInteraction(RPGCharacter character) {
    ...
    double chance =
        MIN_INTERACTION_CHANCE
            + (random.nextDouble() * (MAX_INTERACTION_CHANCE - MIN_INTERACTION_CHANCE));
   ...
   */
  public RPGActionOutcome handleStatInteraction(
      StatInteractionType interaction,
  ...
  private RPGActionOutcome getSuccessOutcome(
      StatInteractionType interaction,
      RPGCharacter character,
      RPGConfig config,
      List<WorldCurse> activeCurses,
    ...
    RPGActionOutcome.Builder builder =
        RPGActionOutcome.builder().xpGained(xpGained).leveledUp(false).success(true);

    String baseNarrative =
        switch (interaction) {
          case FROSTBOUND_BOULDER ->
              FROSTBOUND_BOULDER_SUCCESS[random.nextInt(FROSTBOUND_BOULDER_SUCCESS.length)];
          case FROZEN_GATE -> FROZEN_GATE_SUCCESS[random.nextInt(FROZEN_GATE_SUCCESS.length)];
          case COLLAPSING_ICE_BRIDGE ->
              COLLAPSING_ICE_BRIDGE_SUCCESS[random.nextInt(COLLAPSING_ICE_BRIDGE_SUCCESS.length)];
          case NARROW_CREVICE ->
              NARROW_CREVICE_SUCCESS[random.nextInt(NARROW_CREVICE_SUCCESS.length)];
          case WHISPERING_BARRIER ->
              WHISPERING_BARRIER_SUCCESS[random.nextInt(WHISPERING_BARRIER_SUCCESS.length)];
          case ANCIENT_LIBRARY ->
              ANCIENT_LIBRARY_SUCCESS[random.nextInt(ANCIENT_LIBRARY_SUCCESS.length)];
          case BURIED_CACHE -> BURIED_CACHE_SUCCESS[random.nextInt(BURIED_CACHE_SUCCESS.length)];
          case MYSTERIOUS_GLIMMER ->
              MYSTERIOUS_GLIMMER_SUCCESS[random.nextInt(MYSTERIOUS_GLIMMER_SUCCESS.length)];
          case BLIZZARD_PASSAGE ->
              BLIZZARD_PASSAGE_SUCCESS[random.nextInt(BLIZZARD_PASSAGE_SUCCESS.length)];
          ...
          case FROZEN_GATE -> {
            ...
            if (random.nextDouble() < 0.15) {
              CatalystType catalyst =
                  CatalystType.values()[random.nextInt(CatalystType.values().length)];
  ...
  private RPGActionOutcome getFailureOutcome(
      StatInteractionType interaction,
      RPGCharacter character,
      RPGConfig config,
      List<WorldCurse> activeCurses,
    ...
    RPGActionOutcome.Builder builder =
        RPGActionOutcome.builder().xpGained(xpGained).leveledUp(false).success(false);

    String baseNarrative =
        switch (interaction) {
          case FROSTBOUND_BOULDER ->
              FROSTBOUND_BOULDER_FAILURE[random.nextInt(FROSTBOUND_BOULDER_FAILURE.length)];
          case FROZEN_GATE -> FROZEN_GATE_FAILURE[random.nextInt(FROZEN_GATE_FAILURE.length)];
          case COLLAPSING_ICE_BRIDGE ->
              COLLAPSING_ICE_BRIDGE_FAILURE[random.nextInt(COLLAPSING_ICE_BRIDGE_FAILURE.length)];
          case NARROW_CREVICE ->
              NARROW_CREVICE_FAILURE[random.nextInt(NARROW_CREVICE_FAILURE.length)];
          case WHISPERING_BARRIER ->
              WHISPERING_BARRIER_FAILURE[random.nextInt(WHISPERING_BARRIER_FAILURE.length)];
          case ANCIENT_LIBRARY ->
              ANCIENT_LIBRARY_FAILURE[random.nextInt(ANCIENT_LIBRARY_FAILURE.length)];
          case BURIED_CACHE -> BURIED_CACHE_FAILURE[random.nextInt(BURIED_CACHE_FAILURE.length)];
          case MYSTERIOUS_GLIMMER ->
              MYSTERIOUS_GLIMMER_FAILURE[random.nextInt(MYSTERIOUS_GLIMMER_FAILURE.length)];
          case BLIZZARD_PASSAGE ->
              BLIZZARD_PASSAGE_FAILURE[random.nextInt(BLIZZARD_PASSAGE_FAILURE.length)];
