...
/**
 * Scheduler for boss spawning. Spawns a new boss every 48 hours (24h livable + 24h cooldown) for
 * all servers with RPG enabled.
 */
public class BossScheduler {
  ...
  // Activity-aware spawn: only spawn when players have used RPG commands; pause after 3 inactive
  // cycles
  ...
  public void recordRpgActivity(String guildId) {
    ...
    synchronized (state) {
      ...
      if (state.bossSpawnsPaused) {
        ...
        logger.info(
            "Boss scheduler: Resuming boss spawns for guild {} (RPG command received)", guildId);
      }
    }
  }

  /**
   * Resets activity-aware spawn gating for a guild.
   *
   * <p>Used by `/admin-rpg-setup` to force a fresh start: clears pause/backoff state and marks
   * activity as present so the next scheduler spawn check is allowed to spawn immediately.
   *
   * @param guildId the guild ID
   */
  public void resetRpgSpawnActivity(String guildId) {
    RpgSpawnActivityState state =
        rpgSpawnActivity.computeIfAbsent(guildId, k -> new RpgSpawnActivityState());
    synchronized (state) {
      state.commandsSinceLastSpawn = 1; // allow immediate spawn on next check
      state.inactiveSpawnCycles = 0;
      state.bossSpawnsPaused = false;
    }
  }
  ...
  /**
   * Whether boss spawns are paused for this guild after repeated scheduler cycles with no RPG
   * commands. Used to suppress other automated RPG channel content (e.g. Nilfheim events) until
   * players return.
   *
   * @param guildId the guild ID
   * @return true if paused
   */
  public boolean areBossSpawnsPaused(String guildId) {
    if (guildId == null) {
      return false;
    }
    RpgSpawnActivityState state = rpgSpawnActivity.get(guildId);
    if (state == null) {
      return false;
    }
    synchronized (state) {
      return state.bossSpawnsPaused;
    }
  }

  /**
   * Activity gate for automatic boss spawns. When allowed, consumes the activity slot (resets
   * {@code commandsSinceLastSpawn}). When denied, increments idle cycles and may set {@code
   * bossSpawnsPaused}.
   *
   * @param guildId the guild ID
   * @param guildName the guild name (for logs)
   * @return true if a spawn may proceed
   */
  private boolean tryConsumeBossSpawnActivitySlot(String guildId, String guildName) {
    RpgSpawnActivityState activityState =
        rpgSpawnActivity.computeIfAbsent(guildId, k -> new RpgSpawnActivityState());
    synchronized (activityState) {
      if (activityState.bossSpawnsPaused) {
        logger.debug(
            "Boss scheduler: Skipping guild {} (boss spawns paused, no RPG activity)", guildName);
        return false;
      }
      ...
      try {
        ...
        if (state != null) {
          ...
          if (currentBoss != null) {
            if (currentBoss.isExpired() || currentBoss.isDefeated()) {
              if (currentBoss.isExpired() && !currentBoss.isDefeated()) {
                ...
                if (!tryConsumeBossSpawnActivitySlot(guildId, guildName)) {
                  bossService.validateAndCleanupBossState(guildId);
                  logger.info(
                      "Boss scheduler: Idle gate blocked respawn after normal boss expiry for guild {}",
                      guildName);
                } else {
                  spawnNewBoss(guild, guildId, state);
                }
              } else {
                // Defeated — players engaged; spawn replacement without idle gate
                spawnNewBoss(guild, guildId, state);
              }
            }
            continue; // Boss still active or handled above
          }
          ...
          if (currentSuperBoss != null) {
            if (currentSuperBoss.isExpired() || currentSuperBoss.isDefeated()) {
              if (currentSuperBoss.isExpired() && !currentSuperBoss.isDefeated()) {
                ...
                if (!tryConsumeBossSpawnActivitySlot(guildId, guildName)) {
                  bossService.validateAndCleanupBossState(guildId);
                  logger.info(
                      "Boss scheduler: Idle gate blocked respawn after super boss expiry for guild {}",
                      guildName);
                } else {
                  spawnNewBoss(guild, guildId, state);
                }
            ...
            continue; // Super boss still active or handled above
          }
        }
  ...
  private void pruneOldAnnouncementTracking() {
    Instant cutoff =
        Instant.now()
            .minus(bossService.getMinSpawnIntervalHours(), java.time.temporal.ChronoUnit.HOURS);
    announcedBossIds
        .entrySet()
  ...
  private void announceBossDefeatRewards(Guild guild, BossService.DefeatInfo defeatInfo) {
    ...
    if (!xpRewards.isEmpty()) {
      ...
      for (Map.Entry<String, Integer> entry : xpRewards.entrySet()) {
        ...
        RPGCharacter character = characterService.getCharacter(guild.getId(), userId);
    ...
    RPGCharacter lastHitter = characterService.getCharacter(guild.getId(), lastHitterId);
    if (lastHitter != null) {
