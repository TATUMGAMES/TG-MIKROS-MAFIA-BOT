...
public class RPGCraftCommand implements CommandHandler {
    ...
    public CommandData getCommandData() {
        OptionData itemOption = new OptionData(OptionType.STRING, "item", "Item to craft (optional - leave empty to see list)", false);
    ...
    private void showCraftingList(SlashCommandInteractionEvent event, RPGCharacter character) {
        ...
        for (CraftedItemType itemType : CraftedItemType.values()) {
            ...
            permanentItems.append(String.format("%s %s %s\n",
                    itemType.getEmoji(),
                    itemType.getDisplayName(),
                    status));
        }
        ...
        for (InfusionType infusionType : InfusionType.values()) {
            ...
            infusions.append(String.format("%s %s %s\n",
                    infusionType.getEmoji(),
                    infusionType.getDisplayName(),
                    status));
        }
     ...
     */
    private String formatMaterialStatus(CraftedItemType itemType, com.tatumgames.mikros.games.rpg.model.RPGInventory inventory) {
        ...
        if (essenceHave < essenceNeeded) {
            missing.append(String.format("%s %s ×%d",
                    itemType.getRequiredEssence().getEmoji(),
                    itemType.getRequiredEssence().getDisplayName(),
     ...
     * @return formatted status string
     */
    private String formatInfusionMaterialStatus(InfusionType infusionType, com.tatumgames.mikros.games.rpg.model.RPGInventory inventory) {
        ...
        if (infusionType.isElementalConvergence()) {
            ...
            for (com.tatumgames.mikros.games.rpg.model.EssenceType essence : com.tatumgames.mikros.games.rpg.model.EssenceType.values()) {
                if (inventory.getEssenceCount(essence) < 1) {
                    hasAllEssences = false;
                    break;
                }
            }
            boolean hasCatalyst = inventory.getCatalystCount(com.tatumgames.mikros.games.rpg.model.CatalystType.RUNIC_BINDING) >= 1;
            ...
            if (!hasCatalyst) {
                ...
                missing.append(String.format("%s %s ×1",
                        infusionType.getRequiredCatalyst().getEmoji(),
                        infusionType.getRequiredCatalyst().getDisplayName()));
            }
            missing.append(")");
