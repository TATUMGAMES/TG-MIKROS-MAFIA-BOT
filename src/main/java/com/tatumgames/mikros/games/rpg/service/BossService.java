...
public class BossService {
  ...
  private Boss spawnNormalBossInternal(String guildId, ServerBossState state) {
    ...
    refreshHeroicChargesForGuild(guildId);
  ...
  private SuperBoss spawnSuperBossInternal(String guildId, ServerBossState state) {
    ...
    SuperBoss superBoss =
        BossCatalog.createSuperBoss(definition, level, getLivableIntervalSeconds());
    ...
    if (!topDamage.isEmpty()) {
      ...
      if (totalTopDamage > 0) {
        ...
        if (topDamageDealerId != null) {
          RPGCharacter topDealer = characterService.getCharacter(guildId, topDamageDealerId);
          if (topDealer != null) {
  ...
  /**
   * Removes one player's boss damage total for the current boss cycle. Does not adjust class
   * harmony participation counts.
   *
   * @param guildId the guild ID
   * @param userId the Discord user ID
   */
  public void removePlayerDamage(String guildId, String userId) {
    if (guildId == null || userId == null) {
      return;
    }
    Map<String, Integer> dmg = damageTracking.get(guildId);
    if (dmg != null) {
      dmg.remove(userId);
    }
  }
  ...
  private void refreshHeroicChargesForGuild(String guildId) {
    Collection<RPGCharacter> allCharacters = characterService.getAllCharacters(guildId);
