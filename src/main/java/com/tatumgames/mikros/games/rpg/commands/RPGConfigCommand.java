...
public class RPGConfigCommand implements CommandHandler {
   ...
   * @param rpgGuildResetService orchestrates guild play-state wipes on channel change
   */
  public RPGConfigCommand(
  ...
  private void handleSetChannel(SlashCommandInteractionEvent event, String guildId) {
    ...
    int cleared = rpgGuildResetService.resetPlayProgress(guildId);
        ...
        .reply(
            String.format(
                """
                                ...
                                🧹 **Server RPG state reset** (fresh start): **%d** character profile(s) removed, bosses/curses/events/achievements/auras cleared for this server.

                                Players must use `/rpg-register` again.
                                """,
                channel.getAsMention(), cleared))
        .queue();

    logger.info(
        "RPG channel updated to {} for guild {} (clean-start, cleared {} characters)",
        channel.getId(),
        guildId,
        cleared);
  }
