...
public class AchievementService {
  ...
  /**
   * Removes all first-to achievement claims for a guild (e.g. server fresh start).
   *
   * @param guildId the Discord guild ID
   */
  public void clearGuild(String guildId) {
    ...
    achievementClaims.remove(guildId);
  }

  /**
   * Removes first-to claims in this guild that are held by the given user so they can be earned
   * again after account reset.
   *
   * @param guildId the Discord guild ID
   * @param userId the Discord user ID
   */
  public void releaseClaimsHeldByUser(String guildId, String userId) {
    if (guildId == null || userId == null) {
      return;
    }
    Map<AchievementType, String> guildAchievements = achievementClaims.get(guildId);
    if (guildAchievements == null || guildAchievements.isEmpty()) {
      return;
    }
    guildAchievements.entrySet().removeIf(e -> userId.equals(e.getValue()));
    if (guildAchievements.isEmpty()) {
      achievementClaims.remove(guildId);
    }
  }
}
