...
/**
 * Explore action - player explores the world and encounters random events. Rewards XP and generates
...
public class ExploreAction implements CharacterAction {
    ...
    private final com.tatumgames.mikros.games.rpg.service.StatInteractionService
    ...
    public RPGActionOutcome execute(RPGCharacter character, RPGConfig config) {
        ...
        // Check for class-exclusive exploration event (very rare, 1-2% base chance, separate from
        // negative events)
        ClassExclusiveEventType classEvent = rollForClassExclusiveEvent(character);
        if (classEvent != null) {
            // Class-exclusive event triggered - handle it and return (doesn't interfere with normal
            // exploration)
            return handleClassExclusiveEvent(classEvent, character, config, activeCurses, songReduction);
        }

        // Check for wandering figure encounter (ultra-rare, 0.5% chance, after class-exclusive, before
        // negative events)
        ...
        if (character.getLevel() >= 5
            ...
            com.tatumgames.mikros.games.rpg.exploration.WorldEncounterType encounter =
                    worldEncounterService.rollForIrrevocableEncounter(character);
            if (encounter != null) {
                return worldEncounterService.handleEncounter(
                        encounter, character, config, activeCurses, songReduction);
            }
        }

        // Check for stat-gated interaction (Level 10+, 10-15% chance, max 3 per type)
        if (character.getLevel() >= 10) {
            com.tatumgames.mikros.games.rpg.exploration.StatInteractionType interaction =
                    statInteractionService.rollForStatInteraction(character);
            if (interaction != null) {
                return statInteractionService.handleStatInteraction(
                        interaction, character, config, activeCurses, songReduction);
            }
        }
        ...
        if (negativeEvent != null) {
            ...
            RPGActionOutcome eventOutcome =
                    handleNegativeEvent(negativeEvent, character, config, activeCurses, songReduction);
        ...
        // Apply Curse of Clouded Mind (-5% XP, but ensure minimum 90%) - only if not already handled by
        // event
        ...
        RPGActionOutcome.Builder outcomeBuilder =
                RPGActionOutcome.builder()
                        ...
                        .hpRestored(0)
                        .success(
                                !eventTriggered || damageTaken == 0 || character.getStats().getCurrentHp() > 0);

        // Check for infusion-guaranteed drop
        boolean guaranteedDrop = false;
        if (activeInfusion != null) {
            if (activeInfusion == InfusionType.GALE_FORTUNE
                    || activeInfusion == InfusionType.ELEMENTAL_CONVERGENCE) {
                guaranteedDrop = true;
            }
        }
        ...
        if (guaranteedDrop || random.nextDouble() < dropChance) {
            ...
            if (essence == EssenceType.EMBER_SHARD
                    && character.getCharacterClass()
                    == com.tatumgames.mikros.games.rpg.model.CharacterClass.WARRIOR) {
                benefitNote.append(" Your warrior instincts led you to strength-aligned materials!");
            } else if (essence == EssenceType.MIND_CRYSTAL
                    && (character.getCharacterClass()
                    == com.tatumgames.mikros.games.rpg.model.CharacterClass.MAGE
                    || character.getCharacterClass()
            ...
            } else if (essence == EssenceType.VITAL_ASH
                    && character.getCharacterClass()
                    == com.tatumgames.mikros.games.rpg.model.CharacterClass.KNIGHT) {
                benefitNote.append(" Your defensive nature led you to vitality-aligned materials!");
            } else if (essence == EssenceType.FATE_CLOVER
                    && character.getCharacterClass()
                    == com.tatumgames.mikros.games.rpg.model.CharacterClass.PRIEST) {
        ...
        if (character.getCharacterClass()
            ...
            // Update narrative if not already updated (check both original narrative and any benefit
            // notes)
            String finalNarrative = outcomeBuilder.build().narrative();
            if (!finalNarrative.contains("⚔️💀 **Corruption:**")) {
                outcomeBuilder.narrative(
                        finalNarrative
    ...
    /**
     * Rolls for a negative exploration event. Base chance: 8%, reduced by AGI (-0.2% per AGI) and
     * LUCK (-0.1% per LUCK), minimum 2%.
     *
        ...
        if (random.nextDouble() < finalChance) {
            ...
            if (tierRoll < 0.75) {
                ...
                ExplorationEventType[] tier3Events = {
                        ExplorationEventType.TOUCHED_BY_DARK_RELIC, ExplorationEventType.SEEN_BY_ANCIENT
                };
     ...
     */
    private RPGActionOutcome handleNegativeEvent(
        ...
        switch (eventType) {
            case SLIPPED_ON_ICE -> {
                ...
                narrative =
                        "🧊 **Slipped on Ice:** You lose your footing on a patch of black ice, taking "
                                + damageTaken
            ...
            case PICKPOCKETED_BY_THIEVES -> {
                ...
                if (!availableEssences.isEmpty()) {
                    EssenceType stolenEssence =
                            availableEssences.get(random.nextInt(availableEssences.size()));
                    character.getInventory().removeEssence(stolenEssence, 1); // Remove 1
                    narrative =
                            "🧤 **Pickpocketed by Thieves:** A group of shadowy figures slips past you in the fog. You notice "
                                    + stolenEssence.getDisplayName()
                                    + " is missing from your inventory!";
                } else {
                    narrative =
                            "🧤 **Pickpocketed by Thieves:** A group of shadowy figures slips past you, but your pockets are already empty. Lucky you!";
                }
            }
            ...
            case MOCKED_BY_ANGRY_MOB -> {
                ...
                if (!activeCurses.isEmpty()) {
                    ...
                    if (!availableEssences.isEmpty()) {
                        ...
                        character.getInventory().removeEssence(stolenEssence, 1); // Remove 1
                        narrative =
                                "😠 **Mocked by Angry Mob:** The citizens of Nilfheim blame you for the world's curses! They throw stones and steal "
                                        + stolenEssence.getDisplayName()
                                        + " from your pack.";
                    } else {
                        narrative =
                                "😠 **Mocked by Angry Mob:** The citizens of Nilfheim blame you for the world's curses! They throw stones, but you have nothing left to lose.";
                    }
                } else {
            ...
            case LOST_IN_THE_FOG -> {
                ...
                leveledUp = character.addXp(xpGained, loreRecognitionService);
                narrative =
                        "🌫️ **Lost in the Fog:** A thick mist obscures your path. You eventually find your way, but the disorientation cost you some experience.";
            }
            ...
            case AMBUSHED_BY_CREATURE -> {
                ...
                int enemyLevel =
                        Math.max(1, character.getLevel() - 2 + random.nextInt(5)); // Level-appropriate enemy
                ...
                if (victory) {
                    ...
                    int baseXp =
                            (int)
                                    ((50 + (enemyLevel * 10)) * config.getXpMultiplier() * 0.5); // Half XP for ambush
                    xpGained = baseXp;
                    ...
                    narrative =
                            "🐺 **Ambushed by "
                                    + enemyName
                                    + ":** You were caught off guard, but managed to fight back! You take "
                                    + damageTaken
                                    + " damage but gain "
                                    + xpGained
                                    + " XP.";
                    ...
                    int baseXp =
                            (int)
                                    ((50 + (enemyLevel * 10))
                                            * config.getXpMultiplier()
                                            * 0.25); // Quarter XP for defeat
                    xpGained = baseXp;
                    ...
                    damageTaken =
                            (int) (enemyPower * (0.15 + random.nextDouble() * 0.10)); // 15-25% of enemy power
                    // Apply dark relic damage penalty (check before decrementing)
                    if (hasDarkRelic) {
                        damageTaken = (int) (damageTaken * (1.0 + character.getDarkRelicDamagePenalty()));
                    }
                    ...
                    if (!survived) {
                        ...
                        narrative =
                                "🐺 **Ambushed by "
                                        + enemyName
                                        + ":** The ambush was too much! You fall, defeated. "
                                        + "You gain "
                                        + xpGained
                                        + " XP from the experience, but death claims you.";
                    } else {
                        narrative =
                                "🐺 **Ambushed by "
                                        + enemyName
                                        + ":** You were caught off guard and take "
                                        + damageTaken
                                        + " damage! You barely escape with "
                                        + xpGained
                                        + " XP.";
                    }
                }
            ...
            case FROSTBITE -> {
                ...
                narrative =
                        "🩸 **Frostbite:** The biting cold seeps into your bones. Your maximum HP is reduced by 5% until you rest and warm up.";
            }
            ...
            case TOUCHED_BY_DARK_RELIC -> {
                ...
                narrative =
                        "🕯️ **Touched by a Dark Relic:** An ancient artifact pulses with dark energy as you touch it. "
            ...
            case SEEN_BY_ANCIENT -> {
                ...
                narrative =
                        "👁️ **Seen by Something Ancient:** You feel an otherworldly presence watching you from the shadows. "
            ...
            case ENVIRONMENTAL_HAZARD -> {
                ...
                character.getStats().takeDamage(damageTaken);
                narrative =
                        "⚠️ **Environmental Hazard:** You encounter a dangerous environmental hazard, taking "
                                + damageTaken
                                + " damage. At least you're still alive!";
            }
            ...
            case EXHAUSTION -> {
                ...
                if (random.nextDouble() < 0.05) {
                    ...
                    narrative =
                            "😴 **Exhaustion:** The exploration has left you exhausted. You'll lose an extra action charge on your next action.";
                } else {
                    narrative =
                            "😴 **Exhaustion:** You feel tired from the exploration, but manage to push through.";
                }
            }
            ...
            case TEMPORARY_STAT_DEBUFF -> {
                ...
                if (random.nextDouble() < 0.02) {
                    ...
                    narrative =
                            "📉 **Temporary Stat Debuff:** You've been weakened! Your "
                                    + statDisplayName
                                    + " is reduced by 1 for the next 3 actions.";
                } else {
                    narrative =
                            "📉 **Temporary Stat Debuff:** You feel a momentary weakness, but it passes quickly.";
                }
            }
            ...
            case DANGEROUS_TERRAIN -> {
                ...
                character.getStats().takeDamage(damageTaken);
                narrative =
                        "⛰️ **Dangerous Terrain:** The terrain proves treacherous! You take "
                                + damageTaken
                                + " damage navigating the hazards.";
            }
            ...
            case LOST_TIME -> {
                ...
                if (random.nextDouble() < 0.03) {
                    ...
                    narrative =
                            "⏰ **Lost Time:** You've become disoriented and lost track of time. Your next action will cost 2 charges instead of 1.";
                } else {
                    narrative =
                            "⏰ **Lost Time:** You briefly lose track of time, but quickly regain your bearings.";
                }
            }
            ...
            case CURSED_DISCOVERY -> {
                ...
                if (random.nextDouble() < 0.03) {
                    ...
                    narrative =
                            "🔮 **Cursed Discovery:** You've discovered something cursed! Your XP gain is reduced by 5% for the next 5 actions.";
                } else {
                    narrative =
                            "🔮 **Cursed Discovery:** You sense something dark, but it doesn't affect you this time.";
                }
            }
    ...
    /**
     * Rolls for a rare discovery event. Base chance: 10-15% (randomized per exploration).
     *
     * @param character the character exploring
     ...
     * @return the action outcome
     */
    private RPGActionOutcome handleDiscoveryEvent(
            ExplorationEventType eventType,
            RPGCharacter character,
            RPGConfig config,
            List<WorldCurse> activeCurses,
        ...
        switch (eventType) {
            case ANCIENT_DISCOVERY -> {
                ...
                leveledUp = character.addXp(xpGained, loreRecognitionService);
                narrative =
                        "🏛️ **Ancient Discovery:** You've uncovered an ancient secret! You gain "
                                + xpGained
                                + " XP, but the discovery takes a toll, dealing "
                                + damageTaken
                                + " damage.";
            }
            ...
            case HIDDEN_CACHE -> {
                ...
                if (random.nextDouble() < 0.10) {
                    ...
                    character.getStats().takeDamage(damageTaken);
                    narrative =
                            "💎 **Hidden Cache:** You find a hidden cache of treasures! You gain "
                                    + xpGained
                                    + " XP, but a trap triggers, dealing "
                                    + damageTaken
                                    + " damage!";
                } else {
                    narrative =
            ...
            case RARE_ENCOUNTER -> {
                ...
                int enemyLevel =
                        Math.max(
                                1,
                                character.getLevel() + random.nextInt(3)); // Level-appropriate to slightly higher
                ...
                int playerRoll = random.nextInt(playerPower) + (character.getStats().getLuck() * 2);
                int enemyRoll =
                        random.nextInt(enemyPower)
                                + (enemyLevel * 2); // Less enemy advantage than normal ambush
                ...
                if (victory) {
                    ...
                    leveledUp = character.addXp(xpGained, loreRecognitionService);
                    narrative =
                            "✨ **Rare Encounter:** You encounter a rare "
                                    + enemyName
                                    + " and emerge victorious! You gain "
                                    + xpGained
                                    + " XP but take "
                                    + damageTaken
                                    + " damage.";
                    ...
                    damageTaken =
                            (int) (enemyPower * (0.20 + random.nextDouble() * 0.10)); // 20-30% of enemy power
                    boolean survived = character.getStats().takeDamage(damageTaken);
                    leveledUp = character.addXp(xpGained, loreRecognitionService);
                    ...
                    if (!survived) {
                        character.setIsDead(true);
                        narrative =
                                "✨ **Rare Encounter:** The rare "
                                        + enemyName
                                        + " was too powerful! You fall, defeated. You gain "
                                        + xpGained
                                        + " XP from the experience, but death claims you.";
                    } else {
                        narrative =
    ...
    /**
     * Rolls for a class-exclusive exploration event. Base chance: 1.5% (very rare, separate from
     * negative events).
     *
     * @param character the character exploring
     * @return the event type if triggered, null otherwise
    ...
    /**
     * Rolls for a wandering figure encounter. Base chance: 0.5% (ultra-rare, after class-exclusive,
     * before negative events).
     *
     * @param character the character exploring
     ...
     * @return the action outcome
     */
    private RPGActionOutcome handleWanderingFigure(
            WanderingFigureType figureType,
            RPGCharacter character,
            RPGConfig config,
            List<WorldCurse> activeCurses,
            double songReduction) {
                ...
                switch (outcomeRoll) {
                    case 0 -> {
                        ...
                        if (!availableEssences.isEmpty()) {
                            ...
                            narrative +=
                                    String.format(
                                            "The Merchant trades 2x %s for 1x %s.",
    ...
    /**
     * Handles a class-exclusive exploration event. Matching classes get full rewards, non-matching
     * classes get consolation.
     *
     ...
     * @return the action outcome
     */
    private RPGActionOutcome handleClassExclusiveEvent(
            ClassExclusiveEventType eventType,
            RPGCharacter character,
            RPGConfig config,
            List<WorldCurse> activeCurses,
            double songReduction) {
        ...
        if (isMatchingClass) {
            ...
            RPGActionOutcome consolationOutcome =
                    handleNonMatchingClassEvent(eventType, character, config);
     ...
     */
    private RPGActionOutcome handleMatchingClassEvent(
            ClassExclusiveEventType eventType, RPGCharacter character, RPGConfig config) {
        String narrative = "";
        int xpGained = 0;
        RPGActionOutcome.Builder outcomeBuilder = RPGActionOutcome.builder();
        ...
        switch (eventType) {
            case LOCKED_CHEST -> {
                ...
                narrative =
                        "🔐 **Locked Chest:** Your nimble fingers work the lock with practiced ease. The chest opens!";
            ...
            case HIDDEN_PASSAGE -> {
                ...
                narrative =
                        "🕳️ **Hidden Passage:** You slip through the narrow opening with ease, discovering a hidden route!";
            ...
            case ANCIENT_OATHSTONE -> {
                ...
                narrative =
                        "⚖️ **Ancient Oathstone:** You swear an oath of protection, and the stone responds with ancient power!";
                xpGained = normalXp;

                // Add story flag
                character.addStoryFlag("Swore oath at Ancient Oathstone");

                // Temporary defense bonus would require new tracking field - for now, just XP and story
                // flag
                narrative +=
                        " You feel a protective aura surrounding you. The oath binds you to this place.";
            }
            ...
            case MANA_RIFT -> {
                ...
                narrative =
                        "🌀 **Mana Rift:** You stabilize the chaotic energy, gaining control over the rift!";
                xpGained = (int) (normalXp * 1.30); // 30% XP bonus
            ...
            case MASS_GRAVE -> {
                ...
                narrative =
                        "🪦 **Mass Grave:** You commune with the spirits, learning from their memories!";
                xpGained = (int) (normalXp * 1.25); // 25% XP bonus
            ...
            case DESECRATED_SHRINE -> {
                ...
                narrative =
                        "🕯️ **Desecrated Shrine:** You cleanse the darkness, restoring the shrine's sanctity!";
                xpGained = (int) (normalXp * 1.25); // 25% XP bonus
                ...
                if (random.nextDouble() < 0.20) {
                    ...
                    narrative +=
                            " The restored shrine blesses you with a "
                                    + EssenceType.FATE_CLOVER.getDisplayName()
                                    + "!";
                }
     ...
     */
    private RPGActionOutcome handleNonMatchingClassEvent(
            ClassExclusiveEventType eventType, RPGCharacter character, RPGConfig config) {
        String narrative = "";
        int xpGained = 0;
        ...
        switch (eventType) {
            case LOCKED_CHEST -> {
                narrative =
                        "🔐 **Locked Chest:** You examine the lock, but it's beyond your skill. If only someone more… dexterous were here.";
                character.addStoryFlag("Discovered locked chest but couldn't open it");
            }

            case HIDDEN_PASSAGE -> {
                narrative =
                        "🕳️ **Hidden Passage:** The opening is too narrow for you to slip through. The passage collapses as you attempt to enter.";
            }

            case CRUMBLING_OBSTACLE -> {
                narrative =
                        "🪨 **Crumbling Obstacle:** The barrier is too strong for you to break. You're forced to turn back.";
            }

            case ANCIENT_OATHSTONE -> {
                narrative =
                        "⚖️ **Ancient Oathstone:** The stone doesn't respond to your touch. Only a "
                                + requiredClassName
                                + " could activate its power.";
            }

            case ARCANE_SIGIL -> {
                narrative =
                        "📜 **Arcane Sigil:** The glyphs are incomprehensible to you. Their secrets remain locked to those with magical knowledge.";
            }

            case MANA_RIFT -> {
                narrative =
                        "🌀 **Mana Rift:** The unstable energy is too dangerous for you to approach. A "
                                + requiredClassName
                                + " might be able to stabilize it.";
            }

            case MASS_GRAVE -> {
                narrative =
                        "🪦 **Mass Grave:** You feel unease in this place. The spirits here don't respond to you. You leave quickly.";
            }

            case LINGERING_SPIRIT -> {
                narrative =
                        "👁️ **Lingering Spirit:** The spectral form vanishes as you approach. It seems to only respond to those who understand death.";
            }

            case DESECRATED_SHRINE -> {
                narrative =
                        "🕯️ **Desecrated Shrine:** The darkness here is too strong for you to cleanse. The shrine remains tainted.";
            }

            case LOST_PILGRIM -> {
                narrative =
                        "🕊️ **Lost Pilgrim:** You try to help, but the traveler needs spiritual guidance beyond your ability. A "
                                + requiredClassName
                                + " might be able to comfort them.";
            }
        }
