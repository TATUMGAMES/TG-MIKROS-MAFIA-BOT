...
public class WorldEncounterService {
  ...
  public RPGActionOutcome handleEncounter(
    ...
    return switch (encounterType) {
      case STONEBOUND_DIVINITY ->
          handleStoneboundDivinity(character, config, activeCurses, songReduction);
      case DISGUISED_GOD_TEST ->
          handleDisguisedGodTest(character, config, activeCurses, songReduction);
  ...
  public RPGActionOutcome handleRelicChoice(
    ...
    String discoveryNarrative =
        switch (relic) {
          case BLOOD_FORGED_BLADE ->
              BLOOD_BLADE_DISCOVERY[random.nextInt(BLOOD_BLADE_DISCOVERY.length)];
          case FROZEN_CROWN ->
              FROZEN_CROWN_DISCOVERY[random.nextInt(FROZEN_CROWN_DISCOVERY.length)];
