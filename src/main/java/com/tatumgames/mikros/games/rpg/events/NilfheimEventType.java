...
/**
 * Enum representing server-wide Nilfheim events that affect all players. These events occur every
 * 48-96 hours (randomized) and last for 12 hours.
 */
public enum NilfheimEventType {
  ...
  STARFALL_RIDGE_LIGHT(
      ...
      84,
      96, // Hours between events (84-96, rarest)
  ...
  public String getFormattedEffectDescription() {
    return switch (effectType) {
      case EXPLORE_DROP_BOOST ->
          "+"
              + (int) (effectValue * 100)
              + "% essence drop chance on Explore actions for 12 hours.";
      ...
      case EXPLORE_CATALYST_BOOST ->
          "+"
              + (int) (effectValue * 100)
              + "% catalyst drop chance on Explore actions for 12 hours.";
