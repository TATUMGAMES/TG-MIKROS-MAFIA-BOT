...
/**
 * Command handler for the /warn-suggestions command. Analyzes recent messages and suggests users
 * who may need warnings. Admin-only command.
 */
...
public class WarnSuggestionsCommand implements CommandHandler {
    ...
    public void handle(SlashCommandInteractionEvent event) {
        ...
        int limit =
                (limitOption != null) ? limitOption.getAsInt() : ModerationConfig.MESSAGE_ANALYSIS_LIMIT;
        ...
        logger.info(
                "Analyzing {} messages in channel {} for warn suggestions", limit, channel.getName());
                ...
                .queue(
                        messages -> {
                            List<MessageSuggestion> suggestions =
                                    messageAnalysisService.analyzeMessages(
                                            messages, ModerationConfig.MAX_SUGGESTIONS);

                            if (suggestions.isEmpty()) {
                                event.getHook().sendMessage("✅ No concerning messages found!").queue();
                                return;
                            }
                            ...
                            embed.setDescription(
                                    String.format(
                                            ...
                                            messages.size(), channel.getAsMention(), suggestions.size()));
                            ...
                            logger.info(
                                    "Sent {} warning suggestions to moderator {} in guild {}",
                                    suggestions.size(),
                                    moderator.getId(),
                                    guild.getId());
                        },
