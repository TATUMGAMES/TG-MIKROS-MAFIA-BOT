...
/**
 * Command handler for the /ban command. Bans a user from the server and logs the action. Admin-only
 * command.
 */
...
public class BanCommand implements CommandHandler {
  ...
  public CommandData getCommandData() {
    return Commands.slash("admin-ban", "Ban a user from the server")
        ...
        .addOption(
            OptionType.INTEGER, "delete_days", "Number of days of messages to delete (0-7)", false)
        .setGuildOnly(true)
  ...
  public void handle(SlashCommandInteractionEvent event) {
    ...
    if (deleteDays < 0 || deleteDays > 7) {
      event.reply("❌ Delete days must be between 0 and 7.").setEphemeral(true).queue();
      return;
    }

    // Check role hierarchy only when target is in the server (has a Member); allow ban by User when
    // not
    ...
    // Perform the ban
    guild
        .ban(targetUser, deleteDays, TimeUnit.DAYS)
        .reason(reason)
        .queue(
            success -> {
              String message =
                  String.format(
                      """
                                            ...
                                            Messages deleted: %d day(s)
                                                    """,
                      targetUser.getName(), reason, member.getAsMention(), deleteDays);
              event.getHook().sendMessage(message).queue();

              logger.info(
                  "User {} banned by {} in guild {}: {}",
                  targetUser.getId(),
                  member.getId(),
                  guild.getId(),
                  reason);
            ...
            error -> {
              event
                  .getHook()
                  .sendMessage("❌ Failed to ban user: " + error.getMessage())
                  .setEphemeral(true)
                  .queue();
              logger.error(
