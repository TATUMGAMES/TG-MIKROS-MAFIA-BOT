...
/**
 * Command handler for the /ban-suggestions command. Analyzes recent messages with stricter rules
 * for ban-worthy content. Admin-only command.
 */
...
public class BanSuggestionsCommand implements CommandHandler {
  ...
  public CommandData getCommandData() {
    return Commands.slash("ban-suggestions", "Get suggestions for users who may need bans")
        ...
        .addOption(
            OptionType.INTEGER, "limit", "Number of messages to analyze (default: 200)", false)
        .setGuildOnly(true)
  ...
  public void handle(SlashCommandInteractionEvent event) {
    ...
    int limit =
        event.getOption("limit", ModerationConfig.MESSAGE_ANALYSIS_LIMIT, OptionMapping::getAsInt);

    // Validate limit
    if (limit < 10 || limit > 500) {
      event.getHook().sendMessage("❌ Limit must be between 10 and 500.").setEphemeral(true).queue();
      return;
    }

    logger.info(
        "Analyzing {} messages in channel {} for ban suggestions", limit, targetChannel.getName());
    ...
    targetChannel
        ...
        .queue(
            messages -> {
              List<MessageSuggestion> allSuggestions =
                  messageAnalysisService.analyzeMessages(messages, limit);

              // Filter for HIGH and CRITICAL severity only
              List<MessageSuggestion> banSuggestions =
              ...
              if (banSuggestions.isEmpty()) {
                event.getHook().sendMessage("✅ No severe violations found!").queue();
                return;
              }
              ...
              embed.setDescription(
                  String.format(
                      ...
                      messages.size(), targetChannel.getAsMention(), banSuggestions.size()));
              ...
              for (MessageSuggestion suggestion : banSuggestions) {
                ...
                embed.addField(
                    String.format("#%d - %s", count + 1, suggestion.username()), fieldValue, false);
                count++;
              }

              if (banSuggestions.size() > 10) {
                embed.setFooter(
                    String.format("Showing 10 of %d suggestions", banSuggestions.size()));
              }
              ...
              logger.info(
                  "Sent {} ban suggestions to moderator {} in guild {}",
                  banSuggestions.size(),
                  member.getId(),
                  guild.getId());
            },
            error -> {
              event
                  .getHook()
                  .sendMessage("❌ Failed to fetch messages: " + error.getMessage())
                  .setEphemeral(true)
                  .queue();
