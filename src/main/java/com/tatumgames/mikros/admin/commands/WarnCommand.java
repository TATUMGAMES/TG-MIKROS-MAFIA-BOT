...
/**
 * Command handler for the /warn command. Warns a user and logs the warning in the moderation
 * system. Admin-only command.
 */
...
public class WarnCommand implements CommandHandler {
   ...
   */
  public WarnCommand(
      ModerationLogService moderationLogService, AutoEscalationService autoEscalationService) {
  ...
  public void handle(SlashCommandInteractionEvent event) {
    ...
    ActionType escalationAction =
        autoEscalationService.checkEscalation(targetUser.getId(), guild.getId());
    ...
    if (escalationAction != null) {
      ...
      if (targetMember != null) {
        boolean escalated =
            autoEscalationService.performAutoEscalation(targetMember, guild, guild.getSelfMember());
        ...
        if (escalated) {
          ...
          logger.info(
              "Auto-escalation triggered for user {} in guild {}: {}",
              targetUser.getId(),
              guild.getId(),
              escalationAction);
        }
