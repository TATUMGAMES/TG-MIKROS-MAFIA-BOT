...
/**
 * Command handler for /set-promo-frequency. Allows administrators to set the cooldown for
 * promotional prompts.
 */
...
public class SetPromoFrequencyCommand implements CommandHandler {
    ...
    public CommandData getCommandData() {
        return Commands.slash(
                        "admin-set-promo-frequency", "Set promotional prompt cooldown (admin only)")
    ...
    public void handle(SlashCommandInteractionEvent event) {
        ...
        int days = Optional.ofNullable(event.getOption("days")).map(OptionMapping::getAsInt).orElse(1);

        if (days < 1 || days > 30) {
            event.reply("❌ Cooldown must be between 1 and 30 days.").setEphemeral(true).queue();
            return;
        }
        ...
        embed.setDescription(
                String.format(
                        "Promotional prompt cooldown set to **%d day%s**", days, days == 1 ? "" : "s"));
        ...
        logger.info(
                "Promotional cooldown set to {} days for guild {} by user {}",
                days,
                guildId,
                member.getId());
    }
