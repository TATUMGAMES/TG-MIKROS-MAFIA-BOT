...
public class InMemoryGamePromotionServiceTest {
  ...
  public void testFetchAllApps() {
    ...
    if (firstApp.getCampaign() != null) {
      assertNotNull(
          firstApp.getCampaign().getStartDate(), "Campaign start date should not be null");
      ...
      if (firstApp.getCampaign().getCtas() != null) {
        assertNotNull(
            firstApp.getCampaign().getCtas().getWebsite(), "Website CTA should not be null");
      }
    }
    ...
    assertEquals(
        apps.get(0).getAppId(), apps2.get(0).getAppId(), "Cached result should return same apps");
  }
  ...
  public void testPromotionStepTracking() {
    ...
    assertEquals(
        0, service.getLastPromotionStep(guildId, appId, campaignId), "Should start with step 0");
    assertFalse(
        service.hasAppBeenPromoted(guildId, appId, campaignId), "Should not be promoted initially");
    assertNull(
        service.getLastAppPostTime(guildId, appId, campaignId),
        "Should have no post time initially");
    ...
    assertTrue(
        service.hasAppBeenPromoted(guildId, appId, campaignId), "Should be marked as promoted");
    assertEquals(
        now,
        service.getLastAppPostTime(guildId, appId, campaignId),
        "Should have correct post time");

    // Verify backward compatibility - appId-only methods still work
    assertEquals(
        0,
        service.getLastPromotionStep(guildId, appId),
        "AppId-only should still be 0 (different key)");
  ...
  public void testMultipleCampaignsSameAppId() {
    ...
    assertEquals(
        4,
        service.getLastPromotionStep(guildId, appId, campaignIdJan),
        "January campaign should be at step 4");
    assertTrue(
        service.hasAppBeenPromoted(guildId, appId, campaignIdJan),
        "January campaign should be promoted");

    // Verify February campaign starts fresh (step 0)
    assertEquals(
        0,
        service.getLastPromotionStep(guildId, appId, campaignIdFeb),
        "February campaign should start at step 0");
    assertFalse(
        service.hasAppBeenPromoted(guildId, appId, campaignIdFeb),
        "February campaign should not be promoted yet");
    ...
    assertEquals(
        4,
        service.getLastPromotionStep(guildId, appId, campaignIdJan),
        "January campaign should still be at step 4");
    assertEquals(
        1,
        service.getLastPromotionStep(guildId, appId, campaignIdFeb),
        "February campaign should be at step 1");
    assertEquals(
        now,
        service.getLastAppPostTime(guildId, appId, campaignIdJan),
        "January should have original time");
    assertEquals(
        febTime,
        service.getLastAppPostTime(guildId, appId, campaignIdFeb),
        "February should have new time");
    ...
    assertEquals(
        4,
        service.getLastPromotionStep(guildId, appId, campaignIdFeb),
        "February campaign should be at step 4");
  }
}
