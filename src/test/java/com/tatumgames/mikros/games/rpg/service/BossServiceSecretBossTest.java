...
class BossServiceSecretBossTest {
  ...
  @Mock private AuraService auraService;
  @Mock private WorldCurseService worldCurseService;
  @Mock private NilfheimEventService nilfheimEventService;
  @Mock private LoreRecognitionService loreRecognitionService;
  @Mock private BlessingService blessingService;
  ...
  void shouldSpawnBossWhenMilestoneIsMet() {
    when(characterService.getCharacter(GUILD_ID, USER_ID)).thenReturn(testCharacter);
  ...
  void shouldReturnNullIfCharacterDoesNotExist() {
    when(characterService.getCharacter(GUILD_ID, USER_ID)).thenReturn(null);
    String milestoneKey = "level_10";
  ...
  void shouldHave10PercentChanceForRareCatalyst() {
    ...
    for (int i = 0; i < totalTests; i++) {
      ...
      when(characterService.getCharacter(GUILD_ID, USER_ID + i)).thenReturn(testChar);
